...
// Jonathan Branson -  Lab7.2J_Branson.cpp
...
int main()
{
	...
	while (moreInput == 'y' || moreInput == 'Y')
	{
		...
		for (int count = 0; count < numOfGrades; count++)
		{
			...
			//Input statement to place grade in the array
			cin >> grades[count];
		}

		cout << '\n' << firstname << " " << lastname << " has an average of ";

		// Gets and prints average of student to screen
		average = findGradeAvg(grades, numOfGrades);
		cout << setprecision(2) << fixed << showpoint;
		cout << average;


		// Gets and prints letter grade of student to screen
		cout << " Letter grade is equal to " << findLetterGrade(average)
             << '\n';
...
{
	//If statements that check avg grade to determine letter grade
	//returns the letter to main
